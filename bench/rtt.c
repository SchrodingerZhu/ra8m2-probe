...
__attribute__((used, aligned(4), section(".rtt"))) rtt_cb_t _SEGGER_RTT;
__attribute__((section(".rtt"))) static char up_buf[RTT_UP_SIZE];
__attribute__((section(".rtt"))) static char down_buf[RTT_DOWN_SIZE];
...
void rtt_putc_block(char c)
{
    rtt_ring_t *r = &_SEGGER_RTT.up[0];
    uint32_t wr = r->wroff;
    uint32_t next = wr + 1u; if (next >= r->size) next = 0;
    while (next == r->rdoff) { }              /* wait for host to drain */
    r->buf[wr] = c;
    __asm volatile("dmb" ::: "memory");
    r->wroff = next;
}
